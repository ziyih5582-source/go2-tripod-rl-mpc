...
class MPCController:
    ...
    # 三足步态均匀相位：FL(0) → RL(0.333) → RR(0.667)
    # 均匀分布避免左侧 FL+RL 同时推地时间过长导致的左转
    _PHASE_OFF = {0: 0.000, 2: 0.333, 3: 0.667}
    ...
    def _plan_feet(self,
        ...
        # 大幅放宽 dx_rbt 上限：原 0.015 限制步幅位移仅 0.03m，导致实际速度远低于指令
        # 新上限 0.05 允许步幅位移 0.10m，配合均匀相位实现有效前进
        dx_rbt = min(vx_cmd * t_st * 0.3, 0.05)
...
class BalanceController:
    ...
    # ── PD 增益（保守参数：降低增益减少振荡风险）──────────────
    KP_ROLL  = 0.10    # 原 0.15 [m/rad]    侧向 com 偏移量 / 横滚角
    KD_ROLL  = 0.06    # 原 0.08 [m·s/rad]  微分阻尼
    KP_PITCH = 0.06    # 原 0.10 [m/rad]    纵向 com 偏移量 / 俯仰角
    KD_PITCH = 0.04    # 原 0.06

    # ── 三足站立基准 com_offset ───────────────────────────────
    # NOM_Y = 0.047 将 COM 置于支撑三角形 (FL+RL+RR) 重心 Y 方向
    # 重心计算：Y_centroid = (0.142+0.142-0.142)/3 ≈ 0.047，平衡左右腿 yaw 力矩
    # NOM_X = 0.035 相对重心 X=-0.0645 前移约 0.1m，提供前进偏置
    NOM_X =  0.035
    NOM_Y =  0.047
    NOM_Z =  0.00
    ...
    # ── 急停触发阈值（保守参数：更早触发安全保护）────────────
    SAFE_ROLL  = 0.25   # 原 0.30 rad ≈ 14°
    SAFE_PITCH = 0.20   # 原 0.25 rad ≈ 11°
    SAFE_ROLL_VEL  = 1.0   # 原 1.5 rad/s，角速度过快也急停
    SAFE_PITCH_VEL = 1.0   # 原 1.5
    ...
    def __init__(self, device: str = 'cpu'):
        ...
        self.KI = 0.01          # 原 0.02，降低积分系数，减缓累积
...
if __name__ == '__main__':
    ...
    # 模拟：机器狗向右倾斜 0.1 rad
    print("  模拟右倾 0.1 rad，期望 com_y 增大（大于 NOM_Y=0.047）:")
    for _ in range(30):
        com_out, unsafe = bc.update(roll=0.10, pitch=0.0, gyro_x=0.0, gyro_y=0.0)
    print(f"  com_y = {com_out[0,1]:.4f}（期望 > 0.047）  unsafe={unsafe}")
    assert com_out[0, 1] > 0.047, "ERROR: com_y 应增大！"

    # 模拟：回到水平
    bc.reset()
    print("  模拟机器狗水平（无扰动），期望 com_y 稳定在 0.047:")
    for _ in range(100):
        com_out, unsafe = bc.update(roll=0.0, pitch=0.0, gyro_x=0.0, gyro_y=0.0)
    print(f"  com_y = {com_out[0,1]:.4f}（期望 ≈ 0.047）  unsafe={unsafe}")
    assert abs(com_out[0,1] - 0.047) < 0.001, "ERROR: 应收敛到名义值！"
