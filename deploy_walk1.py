...
自动序列（按Enter后自动执行）：
  1. INITIAL_STAND (3s) → 站起
  2. QUAD_WALK (3s, 0.3m/s) → 四足足行
  3. QUAD_STAND → 停止站立
  4. PRE_SHIFT (2s) → 重心移向支撑三角
  5. LIFT_FR (2.5s) → 抬起FR腿
  6. TRIPOD_STAND (2s) → 三足闭环平衡站立
  7. WALK_START (2s软启动) → 三足行走软启动
  8. WALK (0.15m/s) → 三足行走（保守低速）

使用方法：
  python deploy_walk1.py                          # 纯MPC模式（自动序列）
  python deploy_walk1.py --rl                     # RL+MPC模式（自动序列）
  python deploy_walk1.py --rl --checkpoint=/path/to/model.pt  # 指定模型路径
  python deploy_walk1.py --manual                 # 禁用自动序列，手动键盘控制
  python deploy_walk1.py enp129s0                  # 指定网络接口
...
# ═══════════════════════════════════════════════════════════════
class MPCDeployer:
    # PD增益（激进参数：提高增益以加快响应，适配更高速度）
    KP_STAND  = 50.0   # 站立增益提高
    KD_STAND  = 2.5    # 适当提高阻尼
    KP_TRIPOD = 55.0   # 三足站立增益提高
    KD_TRIPOD = 3.0    # 三足站立阻尼提高
    KP_WALK   = 52.0   # 三足行走增益提高（原42→52，适应提速）
    KD_WALK   = 2.8    # 三足行走阻尼（原2.2→2.8，抑制高速振荡）
    KP_QUAD   = 32.0   # 四足行走增益提高
    KD_QUAD   = 2.0

    # 各阶段时长（激进参数：缩短过渡时间，动作更快）
    T_INITIAL_STAND = 2.0   # 站起时间 2s（原 3.0，更快）
    T_PRE_SHIFT     = 1.0   # 重心移动 1s（原 2.0，加快）
    T_LIFT_FR       = 1.2   # 抬FR腿 1.2s（原 2.5，加快）

    # 行走参数（三足 - 激进参数：提速、抬腿更高、步频更快）
    WALK_VX      = 0.32   # 目标速度 0.32m/s（相比之前 0.22 提速约 45%）
    WALK_STEP_H  = 0.04   # 抬腿高度 0.04m（更高抬腿支撑更大步幅）
    WALK_PERIOD  = 0.95   # 步态周期 0.95s（更快步频）

    # ─── 四足行走参数 ──────────────────────────────────────────
    QUAD_WALK_VX     = 0.4    # 四足前进速度（提高到0.4m/s）
    QUAD_WALK_TIME   = 3.0    # 四足行走持续时间（秒）
    QUAD_STEP_H      = 0.05   # 四足抬腿高度（更高步伐更有力）
    QUAD_GAIT_PERIOD = 0.5    # 四足步态周期（更快步频）
    QUAD_RAMP_TIME   = 0.8    # 四足速度斜坡时间（更快加速）
    TRIPOD_PAUSE_TIME= 0.8    # 三足站立等待时间（缩短停顿）
    ...
    def __init__(self, use_rl=False, rl_checkpoint=None):
        ...
        # 行走相关
        self.vx_cmd = 0.0
        self.walk_ramp_steps = int(0.7 / self.dt)  # 行走启动加速更快（原 1.5s→0.7s）
        self.walk_ramp_cur   = 0
        self.target_vx = self.WALK_VX
        
        # ─── 自动序列计数器 ──────────────────────────────────────
        self.quad_walk_steps  = int(self.QUAD_WALK_TIME / self.dt)   # 四足行走总步数
        self.quad_walk_cur    = 0
        self.quad_ramp_steps  = int(self.QUAD_RAMP_TIME / self.dt)   # 四足速度斜坡步数
        self.quad_ramp_cur    = 0                                      # 四足斜坡计数器
        self.tripod_pause_steps = int(self.TRIPOD_PAUSE_TIME / self.dt)
        self.tripod_pause_cur = 0
        
        # 自动序列控制
        self.auto_mode = True   # True: 自动执行序列  False: 手动键盘控制
    ...
    def _loop(self):
        ...
        # ── 四足静态保持 ───────────────────────────────
            ...
            # ─── 自动序列：站起后自动进入四足行走 ─────────────────
            if self.auto_mode:
                self.ctrl.GAIT_PERIOD = self.QUAD_GAIT_PERIOD   # 0.65s，更稳的步态周期
                self.ctrl.STEP_HEIGHT = self.QUAD_STEP_H        # 0.04m，低抬腿平滑接触
                self.ctrl.reset(torch.tensor([0]))
                self.ctrl.phase[0] = 0.0   # 从0相位干净出发，避免腿突跳
                self.quad_walk_cur = 0
                self.quad_ramp_cur = 0     # 速度斜坡从零开始
                self.state = "QUAD_WALK"
                print(f"[STATE] QUAD_WALK：四足行走启动（目标 {self.QUAD_WALK_VX} m/s，斜坡 {self.QUAD_RAMP_TIME}s）...")

        # ── QUAD_WALK：四足行走（速度斜坡 + 低增益）────────────
        elif self.state == "QUAD_WALK":
            # ① 速度斜坡：smooth_step 从 0 平滑爬升到 QUAD_WALK_VX
            t_ramp = min(self.quad_ramp_cur / max(self.quad_ramp_steps - 1, 1), 1.0)
            cur_vx = self.QUAD_WALK_VX * smooth_step(t_ramp)
            self.quad_ramp_cur = min(self.quad_ramp_cur + 1, self.quad_ramp_steps)

            # ② 轻微前倾 com_offset：让机体重心微微前移，辅助向前蹬地
            com_offset = torch.tensor(
                [[0.02, 0.0, 0.0]],          # x=+0.02m 前倾，y/z保持中立
                dtype=torch.float32, device=self.device)
            ...
            jpos_ctrl, jvel_ctrl = self.ctrl.compute_control(
                ...
                vx_cmd       = cur_vx,
                stepping     = True,
                fr_lift_alpha= 0.0,          # FR腿接地：四足模式
            )
            # ③ 低增益：KP_QUAD=28 / KD_QUAD=1.5，减小冲击力
            self._send_ctrl(jpos_ctrl, jvel_ctrl, self.KP_QUAD, self.KD_QUAD)

            self.quad_walk_cur += 1
            if self.quad_walk_cur >= self.quad_walk_steps:
                self.state = "QUAD_STAND"
                print("[STATE] QUAD_STAND：四足行走完成，过渡到停止...")

        # ── QUAD_STAND：四足减速停止（自动序列）──────────────
        elif self.state == "QUAD_STAND":
            ctrl_dof = self._read_ctrl_dof()
            jpos_ctrl, jvel_ctrl = self.ctrl.compute_control(
                dof_pos      = ctrl_dof,
                com_offset   = torch.tensor([[0.01, 0.0, 0.0]], dtype=torch.float32, device=self.device),
                vx_cmd       = 0.0,
                stepping     = False,
                fr_lift_alpha= 0.0,
            )
            ...
            print("[STATE] PRE_SHIFT：切换到三足站立模式...")
        ...
        # ── TRIPOD_STAND：三足闭环平衡站立 ──────
            ...
            # ─── 自动序列：三足站立保持2秒后自动行走 ─────────
            if self.auto_mode:
                self.tripod_pause_cur += 1
                if self.tripod_pause_cur >= self.tripod_pause_steps:
                    # 开始三足行走
                    ...
                    print(f"[STATE] TRIPOD_PAUSE完成：三足行走开始，RL={'启用' if self.use_rl else '禁用'}...")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--manual', action='store_true', help='禁用自动序列，使用手动键盘控制')
    ...
    print(f"  自动序列: {'禁用(手动模式)' if args.manual else '启用'}")
    ...
    if args.manual:
        dep.auto_mode = False
        print("[INFO] 已禁用自动序列，使用手动键盘控制")
    else:
        print("[INFO] 已启用自动序列：四足足行(3s) → 三足站立(2s) → 三足行走")
