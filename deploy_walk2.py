...
自动序列（按Enter后自动执行）：
  1. INITIAL_STAND (2s) → 站起
  2. QUAD_WALK (3s, 0.5m/s) → 四足足行
  3. QUAD_STAND → 停止站立
  4. PRE_SHIFT → 重心移向支撑三角
  5. LIFT_FR (1.5s) → 抬起FR腿
  6. TRIPOD_STAND (2s) → 三足闭环平衡站立
  7. WALK_START → 三足行走软启动
  8. WALK → 三足行走
...
class MPCDeployer:
    ...
    # ─── 四足行走参数 ──────────────────────────────────────────
    QUAD_WALK_VX     = 0.3    # 四足前进速度（降低到0.3m/s更稳）
    QUAD_WALK_TIME   = 3.0    # 四足行走持续时间（秒）
    QUAD_STEP_H      = 0.04   # 四足抬腿高度（更低更平滑）
    QUAD_GAIT_PERIOD = 0.65   # 四足步态周期（比0.5更稳）
    QUAD_RAMP_TIME   = 1.5    # 四足速度斜坡时间（秒）
    TRIPOD_PAUSE_TIME= 2.0    # 三足站立等待时间（秒）
    ...
    def _loop(self):
        ...
        # ── QUAD_WALK：四足行走（速度斜坡 + 低增益）────────────
            ...
            jpos_ctrl, jvel_ctrl = self.ctrl.compute_control(
                ...
                vx_cmd       = -cur_vx,      # 取反：MPC内部符号约定与前进方向相反
                stepping     = True,
        ...
        # ── WALK：三足行走 ──────
            ...
            jpos_ctrl, jvel_ctrl = self.ctrl.compute_control(
                dof_pos      = ctrl_dof,
                com_offset   = com_offset,
                vx_cmd       = -self.vx_cmd, # 取反：MPC内部符号约定与前进方向相反
                stepping     = True,
                fr_lift_alpha= 1.0,
            )
            self._send_ctrl(jpos_ctrl, jvel_ctrl, self.KP_WALK, self.KD_WALK)
